...
namespace filext
{
	...
	std::filesystem::path findModStorage() {
		...
		if (!GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS |
			...
			(LPCSTR)findModStorage, &hm))
        {
            LOG_G("Fatal Error: GetModuleHandle returned %i\n", GetLastError());
...
#elif defined(__linux__) 
		Dl_info dl_info;
		if (dladdr((void*)findModStorage, &dl_info) == 0) {
	...
	const std::filesystem::path& getStorage() {
		...
		if (storage.empty()) {
			...
			if (!std::filesystem::exists(storage)) {
                LOG_G("Fatal Error: Failed to find a suitable storage folder.\n");
